...
| [`ebay-lister`](./ebay-lister) | 0.1.0 | Builds and publishes eBay listings through conversation. Groups local product photos by unit, optimizes and stages them to Cloudflare R2 with URL verification, resolves the eBay category and its required item aspects from the Taxonomy API, drafts title + item specifics for approval, then runs the Sell Inventory chain (inventory item → offer → publish). Idempotent by SKU, never publishes without confirmation, and can create or edit the business policies and inventory locations a listing depends on. |
