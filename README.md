...
| [`ebay-lister`](./ebay-lister) | 0.2.0 | Builds and publishes eBay listings through conversation, and decides where each SKU should sell. Groups local product photos by unit, stages them to Cloudflare R2 with URL verification, resolves the eBay category and required item aspects, drafts title + specifics in MTGI's HTML house template, then runs the Sell Inventory chain (inventory item → offer → publish; `update` for live edits). Channel check compares eBay vs BrokerBin vs combo net revenue at 3 and 6 months from exact-SKU eBay research plus live BrokerBin asks/RFQs. Idempotent by SKU, never publishes without confirmation, and can create or edit business policies and inventory locations. |
